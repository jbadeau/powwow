 ...
 * @param {Boolean} [args.async=true]
 *     If true, resources are requested asynchronously. Regardless of value,
 *     result is always returned through onComplete callback.
 * @param {Function} args.onComplete
 *     Callback which is invoked if loading the URL was successful,
 *     onComplete( //Object// metadata ).
...
OpenAjax.widget.Loader.prototype.loadMetadata = function( args )
{
    ...
    var _getLocale = function( metadata, onComplete ) {
        ...
        if ( ! localeNode ) {
            if ( metadata.locale.DEFAULT ) {
                lang = "DEFAULT";
                localeNode = metadata.locale.DEFAULT;
            } else {
            ...
            if ( properties.length > 0 ) {
                ...
                for ( var i = 0; i < properties.length; i++ ) {
                    ...
                    if ( properties[i].parentNode.tagName.toLowerCase() === "properties" ) {
                        var attrs = _getAttrs( properties[i].parentNode );
                        if ( attrs.name ) {
    ...
    if (cache && cache[url]) {
        ...
        OpenAjax.widget._xhrGet( u, async, true, _transformXML, args.onError);
    }
};
 ...
 *     the widget spec.
 * @param {Boolean} [args.async=true]
 *     If true, the widget metadata file is loaded asynchronously.
 *     NOTE: When creating the widget on the page, some resources may still be
 *     loaded asynchronously, regardless of this value.
 * @param {Function} args.onComplete
 *     Callback which is invoked if loading the URL was successful,
 *     onComplete( //Object// widgetInstance ).
 * @param {Function} args.onError
 *     Callback which is invoked if an error occurs,
 *     onError( //String// error ).
 * 
 ...
 *     following properties:
 * @param {Object[]} args.widgets
 *     An array of widgets to load. Each array item is an 'args' object as
 *     defined by {@link OpenAjax.widget.Loader#create} (except that the
 *     onComplete and onError properties are ignored -- see below).
 * @param {Function} args.onComplete
 *     Callback which is invoked if loading the URL was successful,
 *     onComplete( //Object[]// widgetInstances ).
 * @param {Function} args.onError
 *     Callback which is invoked if an error occurs,
 *     onError( //String// error ).
 */
OpenAjax.widget.Loader.prototype.createMany = function( args )
{
    var specs = [],
    ...
    function reconcile() {
        ...
        for ( var i = 0; i < specs.length; i++ ) {
            ...
            if ( spec.library ) {
                for ( var libname in spec.library ) {
                    if ( libs[ libname ] ) {
                        var mainLib, workingLib;
                        if ( _vercmp( libs[ libname ].version, spec.library[ libname ].version ) < 0 ) {
                            mainLib = spec.library[ libname ];
                            workingLib = libs[ libname ];
                        } else {
                            mainLib = libs[ libname ];
                            workingLib = spec.library[ libname ];
                        }
                        if ( mainLib.preload || workingLib.preload ) {
                            mainLib.preload = (mainLib.preload || []).concat( workingLib.preload || [] );
                        }
                        if ( mainLib.postload || workingLib.postload ) {
                            mainLib.postload = (mainLib.postload || []).concat( workingLib.postload || [] );
                        }
                        libs[ libname ] = mainLib;
                    } else {
            ...
            if ( spec.require ) {
                if ( reqs.length === 0 ) {
                    ...
                    var reqStart = 0,
                        ...
                        reqLen = reqs.length,
                        changed = false;
                    for ( var j = 0; j < spec.require.length; j++ ) {
                        var r = spec.require[j];
                        if ( ! r.src ) {
                            continue;
                        }
                        ...
                        if ( idx !== -1 ) {
                            changed = true;
                            reqs = reqs.slice( 0, idx ).concat( spec.require.slice( specReqStart, j ), reqs.slice( idx ) );
                            reqStart = idx + 1;
                            specReqStart = j + 1;
                        }
                    }
        ...
        function _loadWidgetAtIndex( idx, onComplete, onError ) {
            ...
            w.spec = specs[ idx ];
            w.onComplete = function( instance ) {
                instances[ idx ] = instance;
                if ( --created === 0 ) {
                    args.onComplete( instances );
                }
                if ( onComplete ) {
                    onComplete( instance );
                }
 ...
 *     The widget metadata object, constructed by
 *     {@link OpenAjax.widget.Loader#loadMetadata}.
 ...
 */
function _doValueSubstitutions( text, metadata, properties )
{
    ...
    if ( locale ) {
        text = text.replace( /##(\w+)(?:\((\w+)\))?##/g,
                function( str, p1, p2 ) {
                    var key = p2 || p1;
                    var newText = locale.messages[ key ];
                    if ( newText ) {
                        if ( p2 ) {
                            switch ( p1 ) {
                                case "escapequotes":
                ...
                function( str ) {
                    switch ( str ) {
                        case "__BIDI_START_EDGE__":
                            return locale.language_direction === "ltr" ? "left" : "right";
                        case "__BIDI_END_EDGE__":
                            return locale.language_direction === "ltr" ? "right" : "left";
                        case "__BIDI_DIR__":
                            return locale.language_direction;
                        case "__BIDI_REVERSE_DIR__":
                            return locale.language_direction === "ltr" ? "rtl" : "ltr";
                    }
                    return str;
                }
        );
    ...
    // do property value substitutions
    return text.replace( /@@(\w+)(?:\((\w+)\))?@@/g,
            function( str, p1, p2 ) {
                var name = p2 || p1;
                var newText = (properties && properties[name]) ||
                        (metadata.property[name] && metadata.property[name].defaultValue);
                if ( newText ) {
                    if ( p2 ) {
                        switch ( p1 ) {
                            case "escapequotes":
...
//            }
            ...
            switch ( req.type ) {
                case "javascript":
                    scripts.push( uri ?
                            { src: uri } :
                            { text: req._content_.replace(/__WID__/g, this._id) }
                    );
                    break;
                case "css":
...
console.error( error );
    ...
    _loadContent: function( target, mode )
    {
        ...
        if ( content.src ) {
            ...
            this._widgetContext.loadedCallbacks++;
            OpenAjax.widget._xhrGet( uri, true, false,
                function(text){
                    text = _doValueSubstitutions( text.replace(/__WID__/g, that._id),
                            that._spec, that._properties );
            ...
            var text = content._content_.replace(/__WID__/g, this._id);
                    ...
                    function( scripts, success, error ) {
                        if ( success ) {
                            ...
                            for ( var i = 0; i < scripts.length; i++ ) {
                                var text = scripts[i].text.replace(/__WID__/g, that._id);
                                if ( scripts[i].src ) {
                                    text = _doValueSubstitutions( text, that._spec, that._properties );
                                }
                                var script = {
                                    text: text,
                                    src: scripts[i].src
                                ...
                                switch ( scripts[i].location ) {
                                    case "beforeContent":
                                        that._widgetContext.body.jsBefore[ jsBefore++ ] = script;
                                        break;
                                    case "atEnd":
                                        that._widgetContext.body.jsEnd[ jsEnd++ ] = script;
                                        break;
                                    default:
                                    //case "afterContent":
                                        that._widgetContext.body.jsAfter[ jsAfter++ ] = script;
                                        break;
                                }
                            }
                            that._widgetContext.runDeferredScripts();
...
//     matching <script> in comments.
        var that = this;
        return text.replace(/<script\s*(?:[^>]*?(?:src=(['"]?)([^>]*?)\1[^>]*)?)*>([\s\S]*?)<\/script>/gi,
                function ( ignore, delim, src, code ) {
                    if (src) {
                        OpenAjax.widget._xhrGet( src, true, false,
                            function(text){
                                that._widgetContext.body.jsContent.push({ text: text, src: src });
                            },
                            function(err) {
                        ...
                        that._widgetContext.body.jsContent.push({ text: code });
                    }
                    return "";
                }
        );
 ...
 * @param {Boolean} async
 *     If true, the request is handled asynchronously.
 * @param {Boolean} forceXml
 *     Treat URL contents as XML. If true, the contents are returned as a DOM
 *     tree representing the XML contents. Otherwise, the contents are returned
 *     as a string.
 * @param {Function} onComplete
 *     Callback which is invoked if retrieving the URL contents was successful,
 *     onComplete( //HTMLElement | String// contents ).
 * @param {Function} onError
 *     Callback which is invoked if an error occurs,
 *     onError( //String// error ).
 */
oaw._xhrGet = function( url, async, forceXml, onComplete, onError )
{
        ...
        function evalNextScript( idx ) {
            if ( idx === nextIdx ) {
                while ( nextIdx < scripts.length && scripts[ nextIdx ].text ) {
                    if ( doEval ) {
                        oaw._runScript( scripts[ nextIdx ].text, scripts[ nextIdx ].src );
                    }
                    nextIdx++;
                }
 ...
 * @param {String} [source]
 *     Source of the JavaScript code. Used for debugging purposes.
 */
    ...
    if ( window.eval ) {
        ...
        if ( window.__openajax_widget_fn__ ) {
            ...
            return function( text, source ) {
                if ( source ) {
                    text += "\r\n//@ sourceURL=" + source;  // debugging aid
                }
                window.eval( text );
            };
        }
